...
| `Select` dropdown nested inside a popover nested inside another popover | `10005` | `OutboundAddButton`'s detail form (Select Channel / Select Phone / Outbound Skill), set via `Select`'s own `dropdownClassName` prop — must clear its own `z-[10003]` parent popover |
