...
Every communication channel type gets one fixed, consistent color, rather than every consuming app picking its own tint per instance. `ChannelType` itself (`channel-row.tsx`) has five values — Chat/SMS/WhatsApp are three genuinely distinct channels (a website chat widget, texting, and a WhatsApp thread are three different conversations an agent can have with the same customer at once), each with its own color, not one shared "text channel" color the way an earlier version of this table had it:

| Channel type | Color | `Tag` variant | Token pair |
|---|---|---|---|
| Voice | Purple | `variant="purple"` | `lyra-accent-purple-{soft,strong}` |
| Chat | Teal | `variant="teal"` | `lyra-accent-teal-{soft,strong}` |
| SMS | Neutral (gray) | `variant="neutral"` | `lyra-bg-surface-canvas` / `lyra-fg-secondary` |
| WhatsApp | Default (blue) | `variant="default"` | `lyra-bg-active-subtle` / `lyra-fg-active-strong` |
| Email | Pink | `variant="pink"` | `lyra-accent-pink-{soft,strong}` |

Voice/Chat/Email keep the three dedicated `lyra-accent-*` hues (purple/teal/pink) `Tag` exposes specifically for this purpose (not the full `lyra-accent-*` set — slate/red/orange/yellow/lime/green/blue exist as tokens but aren't wired into `Tag`). SMS/WhatsApp reach past that three-hue set into `Tag`'s two general-purpose, non-status variants (`neutral`/`default`) instead — there's no fourth/fifth dedicated accent hue to give them without reusing a status color (see below), and this at least keeps every channel visually distinct without inventing an unwired token.

**Only `agent-next-gen-v1`'s Contact History still collapses SMS/WhatsApp into "Chat"** — `contactHistoryChannelType`/`ContactHistoryEntry.channelType` there is a genuinely narrower, three-value grouping (`"voice" | "chat" | "email"`, no SMS/WhatsApp values exist in that type at all), a deliberate simplification specific to that one list, not evidence that SMS/WhatsApp share a color anywhere else. Don't use it as a reference for how `ChannelType`'s own five values should be colored.

Pass an `icon` (a Lucide glyph, or `WhatsAppIcon` — the real brand glyph, not `MessageCircle`, see `channel-row.tsx` — for WhatsApp specifically) alongside `label`/`variant`/`shape="pill"` for a channel-type pill (see `agent-next-gen-v1`'s `ContactHistoryCard`, the reference implementation this convention was pulled from — though see the previous paragraph for why its own SMS/WhatsApp-as-"Chat" grouping doesn't extend past that one file). Where there's no room for a full pill (e.g. a 48px icon-only table column), tint the bare icon with the matching `text-lyra-accent-{purple,teal,pink}-strong` (or, for SMS/WhatsApp, `text-lyra-fg-secondary`/`text-lyra-fg-active-strong`) class instead of leaving it flat gray (see `AgentNextGenPage.tsx`'s `InteractionsTable` type-icon column) — same colors, just without the pill chrome.
...
| Popovers nested inside another popover | `10003` | `CreateNew`'s per-row channel flyout (Outbound picker); `PhoneInput`'s country dropdown when used inside `CreateNew` (dialpad group and drill-down screen 1, via its `dropdownClassName` prop); `ChannelRow`'s "Log Outcome" popover, reused verbatim inside `InteractionNavItem`'s compact-tile hover-preview popover — both must clear their own `z-[9999]`/`z-50` parent panel |
...
| `Select`/`Popover` dropdown nested inside a popover nested inside another popover | `10005` | `OutboundAddButton`'s detail form (Select Channel / Select Phone / Outbound Skill), set via `Select`'s own `dropdownClassName` prop; `ChannelRow`'s "Log Outcome" popover's own Resolution popover, Tags `Select`, and Disposition code `Select` — all must clear their own `z-[10003]` parent popover |
...
- **`Select`'s own dropdown (`Select.Content` in single-select mode, `Popover`'s content in multi-select mode) is the same failure mode one level deeper still.** `OutboundAddButton`'s detail form (create-new.tsx) renders three `Select`s inside its own `z-[10003]` "popover nested inside another popover" panel — `Select`'s default `z-[9999]` sits below that parent, so an open dropdown painted underneath it (invisible, or visible only past the parent panel's own edges — this was the reported symptom: the "Outbound Skill" dropdown appeared detached below the whole card instead of under its own field, and the "Select Channel"/"Select Phone" dropdowns, which would have opened *inside* the parent panel's bounds, didn't appear at all). Fixed the same way as `PhoneInput`/`Menu` above: a `dropdownClassName` prop on `Select`, merged onto its dropdown's className, raised to `z-[10005]` at this specific nested call site — the next unused integer.
