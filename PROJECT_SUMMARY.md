...
## Known Debt
- **Menu/Popover width scale not yet fully retrofitted.** `CONTRIBUTING.md` now documents a `sm`/`md`/`lg` → `200px`/`256px`/`320px` width scale for `Menu`/`Popover`-based item-list panels (added when `new-outbound.tsx` and `agent-profile.tsx` were confirmed to already match `lg`/`md` respectively). Components still using an unreviewed one-off width and not yet checked against the scale: `select.tsx` (`w-[240px]`), `connected-apps.tsx` (`w-72`), `ai-input.tsx`/`profile-menu.tsx` (`min-w-[220px]`). `app-menu.tsx` is a bigger case — it's `w-[320px]` (already matches `lg`) but also hand-rolls its own `role="menu"` markup instead of composing the real `Menu` component (the same composition-over-reimplementation issue `new-outbound.tsx` had before it was fixed to render `<Menu>` internally). Fix `app-menu.tsx`'s composition first, then re-check its width against the scale as part of that pass — don't just resize it in place.
