...
/* ── QuickReplyMenu ──
   The "#trigger" list a chat composer shows while the agent is mid-typing
   a quick-reply shortcut (e.g. `#time` while typing "#timeline") — built
   as a plain, presentational overlay rather than a `Popover`/`Menu`
   composition, because the thing that must keep DOM focus the whole time
   this is open is the composer's own `<textarea>`, not this list: arrow
   keys/Enter/Escape all need to keep typing into the textarea's own
   `onKeyDown` handler (so the caret position and the rest of the message
   text around the "#token" stay exactly where the agent left them), which
   rules out `Menu`/`Autocomplete`'s own internally-owned open/keyboard
   state — this component only ever reflects `activeIndex`/`items` the
   caller already computed, and reports hover/click back up rather than
   managing any of that itself.

   Caller is responsible for positioning this (e.g. an `absolute
   bottom-full` box above the composer, matching the reference mockup —
   this component has no opinion on where it sits, only its own internal
   layout/chrome). */
...
const QuickReplyMenu = React.forwardRef<HTMLDivElement, QuickReplyMenuProps>(
  ({ query, items, activeIndex, onSelect, onHoverItem, onClose, emptyMessage = "No matching quick replies", className }, ref) => {
    return (
      <div
        ref={ref}
        role="listbox"
      ...
      >
        <div className="flex shrink-0 items-center justify-between gap-3 border-b border-lyra-border-subtle bg-lyra-bg-surface-container-subtle px-4 py-2">
          <span className="lyra-body-sm text-lyra-fg-secondary truncate">
            Matching <span className="font-mono text-lyra-fg-active-strong">#{query}</span>
