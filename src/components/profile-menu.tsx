...
const ProfileMenu = React.forwardRef<HTMLDivElement, ProfileMenuProps>(
  ({ className, initials, avatarColor = "#5d6a79", groups, ...props }, ref) => {
          ...
          >
            {groups.map((group, gi) => (
              <React.Fragment key={gi}>
                {gi > 0 && (
                  <div className="my-1 border-t border-lyra-border-subtle" />
                )}
                <div className="flex flex-col">
                  {group.items.map((item, ii) => (
                    <button
                      ...
                      className="flex w-full items-center gap-2.5 px-4 py-2.5 text-left lyra-body-md text-lyra-fg-default transition-colors hover:bg-lyra-state-hover active:bg-lyra-state-pressed"
                    >
                      {item.icon && (
